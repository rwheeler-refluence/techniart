...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_nestle where sess='$sess' && status='open'";
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_nestle where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_nestle values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_nestle set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
...
$sql="select * from tblotsdetail_nestle where otsID='$o'";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select modelNumber from tblProducts_nestle where productID='$itemNo'";
...
if (mysql_num_rows($product5)) {
	...
	$product13 = mysql_query("SELECT qty FROM tblotsdetail_nestle WHERE otsID = '$o' && itemNo='13' ");
