...
$sql="select * from tblotsdetail_nestle where otsID='$o' ORDER BY ItemNo + 0 ASC";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		if(!$_SESSION['zip']){
			$zz=$_POST['ship_zip'];
			$sqlz="select * from tblDiscounts LEFT OUTER JOIN tblProducts_nestle on tblDiscounts.item_no=tblProducts_nestle.modelNumber where tblProducts_nestle.productID='$itemNo' && zip='$zz'";
			#print($sqlz);
			...
			if($countz>0){
				while($rowz=mysql_fetch_array($resultz)){
					...
					$sqlu="update tblotsdetail_nestle set price='$disct_price' where otsdetailID='$otsdetailID'";
					#print($sqlu);
...
$rate = '0.00';
#$sqltest = "SELECT ItemNo FROM tblotsdetail_nestle WHERE otsID='$o' ORDER BY ItemNo + 0 ASC";
#$sqltest = "SELECT ItemNo FROM tblotsdetail_nestle WHERE otsID='$o' ORDER BY ItemNo ASC";
