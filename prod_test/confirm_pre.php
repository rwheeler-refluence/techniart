...
$instr=$_POST['instr'];
$order=$ship_fname . ',' . $ship_lname . ',' . $ship_address1 . ',' . $ship_address2 . ',' . $ship_city . ',' . $ship_state . ',' . $ship_zip . ',' . $fname . ',' . $lname . ',' . $address1 . ',' . $address2 . ',' . $city . ',' . $state . ',' . $zip . ',' . $source . ',' . $email_opt . ',' . $email . ',' . $water . ',' . $people . ',' . $otsID;
//$orderinfo = str_replace("'", "&#39;", $order);
$orderinfo = mysql_real_escape_string($order);
//$orderinfo = urldecode($order);
?>
<?
$ship_zip=$_POST['ship_zip'];
...
if($countzip<1){echo "<SCRIPT LANGUAGE='JavaScript'>
	window.alert('The shipping zip code you entered does not qualify!')
		window.location.href='https://www.techniart.us/masssave/orderform.php'
        </SCRIPT>
        ";
		die();}
...
echo "******</br>";
$result=db_query($sql);
echo mysql_fetch_array($result)['orderinfo'];
