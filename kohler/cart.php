...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend_foe where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail_foe where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail_foe values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail_foe set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail_foe where otsdetailID='$otsdetailID[$i]'";
			}else{
...
<?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail_foe LEFT OUTER JOIN tblProducts_foe on tblotsdetail_foe.itemNo=tblProducts_foe.productID where tblotsdetail_foe.otsID='$o'";
#print($sql);
$result=db_query($sql);
...
if(!$count){
 ...
 while($row=mysql_fetch_array($result)){
	...
	/*	if(strlen($_SESSION['zip_qualify'])>0){
			$pr=$row['disct_price'];
			if($pr!==$price){
			$sqlu="update tblotsdetail_foe set price='$pr' where otsdetailID='$otsdetailID'";
...
#		print("".$calcweight."<br>");
		...
		$sqlfreeship="select free_ship from tblProducts_foe where productID='$productID'";
...
#5/21
		#shipping actuals
		$sqlship="select * from tblProducts_foe where productID='$itemNo'";
	//	print("".$sqlship."<br>");
...
if (mysql_num_rows($product5)) {
    ...
    $PQ10=$P10['qty']*1;}
		...
		$product11 = mysql_query("SELECT qty FROM tblotsdetail_foe WHERE otsID = '$o' && itemNo='11' ");
	if (mysql_num_rows($product11)) {
    $P11 = mysql_fetch_assoc($product11);
    $PQ11=$P11['qty']*4;}
		$product12 = mysql_query("SELECT qty FROM tblotsdetail_foe WHERE otsID = '$o' && itemNo='12' ");
	if (mysql_num_rows($product12)) {
    $P12 = mysql_fetch_assoc($product12);
    $PQ12=$P12['qty']*6;}
	$bulblimit=$PQ1+$PQ2+$PQ3+$PQ4+$PQ5+$PQ6+$PQ7+$PQ8+$PQ9+$PQ10+$PQ11+$PQ12;
	$product13 = mysql_query("SELECT qty FROM tblotsdetail_foe WHERE otsID = '$o' && itemNo='26' ");
...
if (mysql_num_rows($product242)) {
    ...
    $PQ242=$P242['qty']*1;}
	$product243 = mysql_query("SELECT qty FROM tblotsdetail_foe WHERE otsID = '$o' && itemNo='243' ");
