...
if($vendorID){
	print("<br>");
	$sql="select * from tblOrdersCompleted LEFT OUTER JOIN tblTerritory on tblTerritory.zip=tblOrdersCompleted.ship_zip LEFT OUTER JOIN tblVendors on tblTerritory.vendor=tblVendors.vendorID where tblOrdersCompleted.stamp BETWEEN '$start' AND '$end' && tblTerritory.vendor='$vendorID' order by tblOrdersCompleted.completeID desc";
#	print($sql);
	$result=db_query($sql);
