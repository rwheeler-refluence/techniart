...
$sqlp.="select * from tblProducts_copy where category='$cat' && prodclass='$prodclass'";
if($subcat){
$sqlp.="&& subcat1='$subcat'";
	}
	if($subcat1){
...
$i=1;
	...
	print("</b></span><br>");
	if(!$subcat){
		$sqlsubs="select distinct subcat1 from tblProducts_copy where category='$cat' && subcat1!='' order by $sort asc";
		...
		$sqlsubs="select distinct subcat2 from tblProducts_copy where subcat1='$subcat' && category='$cat' && subcat2!='' order by subcat2 asc";
		$resultsubs=db_query($sqlsubs);
		$countsubs=mysql_num_rows($resultsubs);
		$subs=1;
		while($rowsubs=mysql_fetch_array($resultsubs)){
...
while($rowp=mysql_fetch_array($resultp)){
	...
	$modelNumber=$rowp['modelNumber'];
			$loc=$rootDir."efficiencyunited/pix/products/thumbnails/".$imageLoc;
			if(!file_exists($loc)){
				$loc=$rootDir."efficiencyunited/pix/products/".$imageLoc;
				$folder="efficiencyunited/pix/products/";
			}else{
...
?>
<td width="275">
<form method="post" action="https://www.techniart.us/efficiencyunited/cart.php">
