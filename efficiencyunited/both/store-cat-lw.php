...
$sqlp.="select * from tblProducts_copy where category='$cat' ";
if($subcat){
$sqlp.="&& subcat1='$subcat'";
	}
	if($subcat1){
...
<? if($nomsrp=='Yes'){?>
<? if($_SESSION['util']>16){?><span class="product_price_z"><? }else{?><span class="product_price"><? }?><strike>$<? echo($MSRP);?></strike></span><br>
<?}?>
<? if($_SESSION['util']>16){?><span class="product_price_z"><? }else{?><span class="product_price"><? }?>$<? echo($price);?></span><br>
<? if(!$_SESSION['']){?>
