...
<title>Duke Energy Progress - Cart</title>
...
</head><BODY><?php include_once("analyticstracking1.php") ?><?php include("nav-lto.php") ?>

<center><div class="gridContainer clearfix"><div id="LayoutDiv1"><br>

	<?php include_once("header-lto.php") ?>
	<?php include_once("header1.php") ?></div>
...
$par38 = mysql_query("SELECT qty FROM tblotsdetail WHERE otsID = '$o' && itemNo='242' ");
if (mysql_num_rows($par38)) {
    $P1 = mysql_fetch_assoc($par38);
    $PQ1=$P1['qty']*1;}
$can = mysql_query("SELECT qty FROM tblotsdetail WHERE otsID = '$o' && itemNo='243' ");
if (mysql_num_rows($can)) {
    $P2 = mysql_fetch_assoc($can);
    $PQ2=$P2['qty']*1;}
$med = mysql_query("SELECT qty FROM tblotsdetail WHERE otsID = '$o' && itemNo='244' ");
if (mysql_num_rows($med)) {
    $P3 = mysql_fetch_assoc($med);
    $PQ3=$P3['qty']*1;}
$canlimit=$PQ2+$PQ3;
print("<div>");
if(!$count){
...
print("<form  method=\"post\" action=\"".$PHP_SELF."\">\n");
	...
	while($row=mysql_fetch_array($result)){
		...
		print("<td align=\"center\"><input type=\"button\" class=\"btn2\" value=\"Update\" onClick=\"this.form.submit();\"> <input type=\"button\" class=\"btn2\" value=\"Remove\" onClick=\"location.href='remove.php?ID=".$otsdetailID."'\"></td>\n");
...
#	print("</tr>\n");
	...
	print("</tr></form>\n");
	print("<tr>\n");
	print("<td colspan=\"5\" align=\"right\"><input type=\"button\" class=\"btn1\" value=\"Empty Cart\" onclick=\"location.href='http://www.techniart.us/dukeenergy/empty-lto.php?otsID=".$o."'\"></td>\n");
	print("</tr>\n");
		#print("<tr>\n");
	#print("<td colspan=\"5\" align=\"right\"><input type=\"button\" class=\"btn1\" value=\"Everyday Store\" onclick=\"location.href='http://www.techniart.us/dukeenergy/store.php'\"></td>\n");
	#print("</tr>\n");
	
	print("<tr>\n");
	if($PQ1 > '2'){
	print("<td colspan=\"5\" align=\"right\" class=\"cart\">In order to checkout, please adjust your cart to <b>2 total Par38 packs
 or less</b>.</td>\n");	
	print("</tr>\n");
	print("<tr height='25px'><td></td></tr>\n");
	print("</table>\n");
	}else{
	if($canlimit > '2'){
	print("<td colspan=\"5\" align=\"right\" class=\"cart\">In order to checkout, please adjust your cart to <b>2 total Candle packs or less</b>.</td>\n");	
	print("</tr>\n");
	print("<tr height='25px'><td></td></tr>\n");
	print("</table>\n");
	}else{
		print("<td colspan=\"5\" align=\"right\"><form method=\"post\" action=\"https://www.techniart.us/dukeenergy/orderform-lto.php\"><input type=\"hidden\" name=\"source\" value=\"".$source."\"><input type=\"hidden\" name=\"otsID\" value=\"".$o."\"><input type=\"hidden\" name=\"ship\" value=\"".$ship."\"><input type=\"hidden\" name=\"state\" value=\"".$_SESSION["shipping"]["state"]."\"><input type=\"submit\" class=\"btn1\" value=\"Checkout\"></td>\n");
	print("</tr></form>\n");
