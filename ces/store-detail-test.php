...
<? include("zip-test.php");?><br>

<!-- ------------------------------begin products -detail------------------------------ -->
<?
$sqlp="select * from tblProducts_copy where productID='$ID'";
...
if($replacement_bulb){
	...
	for($r=0;$r<count($split);$r++){
			...
			$sqlnum="select productID,productName from tblProducts_copy where modelNumber='$mf_nbr'";
