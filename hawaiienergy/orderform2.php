...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_nstar where sess='$sess' && status='open'";
		$resultc=db_query($sqlc);
		$countc=mysql_num_rows($resultc);
		if(!$countc){
			$sql="insert into tblorderstosend_nstar values ('','$sess','','','$stamp','open')";
			$result=db_query($sql);
		...
		$_COOKIE['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_nstar where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_nstar values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_nstar set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			$sql="update tblotsdetail_nstar set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
#print("".$sql."<br />");			
			$result=db_query($sql);
		}
	break;
...
<html>
<head>
<title>TechniArt - NSTAR Earth Day Sale</title>
...
document.form1.country.value=document.form1.ship_country.selectedIndex;
	...
	<!--
		function fVerifyResForm() {
			...
			if (oForm.ship_country.value=='') {
				errFlag = 1;
				errStr = errStr + "Please enter your office location.\n";
			}
...
<td width="906" class="title_bkg"><div id="title_spacer" align="left"><span class="title_main">NSTAR Earth Day Sale</span></div></td>
</tr></table>
...
$sql="select * from tblotsdetail_nstar where otsID='$o'";
$result=db_query($sql);
#print($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select free_ship from tblProducts_nstar where productID='$itemNo'";
		...
		print("<td><a class=\"body_content_style1\" href=\"#\" onClick=\"javascript: decision('Are you sure you want to remove this item from your shopping bag?', 'http://www.techniart.com/progress/remove.php?ID=".$otsdetailID."');\"><span class=\"body_content_style1\">REMOVE?</span></a></td>\n");
...
<td width="255"><input type="text" size="25" maxlength="100" name="ship_fname" value="" class="forms3" /></td>
</tr><tr valign="top">
...
<tr valign="top">
  ...
  <td><textarea name="instr" id="instr" rows="5" cols="30" class="forms3" />
      </textarea></td>
</tr>
<tr valign="top">
<td colspan="2"><img src="pix/pix_trans.gif" width="1" height="5"></td>
...
<td width="152"><input type="text" size="25" maxlength="100" name="fname" value="" class="forms3" /></td>
</tr><tr valign="top">
<td colspan="2"><img src="pix/pix_trans.gif" width="1" height="5"></td>
</tr><tr valign="top">
<td><span class="body_content_style1">Last Name:</span></td>
