...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend_amigo where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
		$resultc1=db_query($sqlc1);
		$countc1=mysql_num_rows($resultc1);
		if($countc1<1){
			$sql="insert into tblorderstosend_amigo values ('','$sess','','','$stamp','open')";
#print("c2: ".$sql."<br>");
			$result=db_query($sql);
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail_amigo where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail_amigo values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail_amigo set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail_amigo where otsdetailID='$otsdetailID[$i]'";
			}else{
				$sql="update tblotsdetail_amigo set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
			}
			$result=db_query($sql);
		}
	break;
...
<?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail_amigo LEFT OUTER JOIN tblProducts on tblotsdetail_amigo.itemNo=tblProducts.productID where tblotsdetail_amigo.otsID='$o'";
$sqlo="select sum(qty) as TotalQuantity from tblotsdetail_amigo where tblotsdetail_amigo.otsID='$o'";
#print($sql);
#print($sqlo);
$result=db_query($sql);
...
#	print("</tr>\n");
	...
	print("<tr><td colspan=\"4\" height=\"5\" align=\"right\"></td></tr>\n");
	print("<tr>\n");
	print("<td colspan=\"4\" align=\"right\"><input type=\"button\" class=\"btn2\" value=\"Continue Shopping\" onclick=\"location.href='http://www.techniart.us/amigo/store.php'\"></td>\n");
	print("</tr>\n");
	print("<tr><td colspan=\"4\" height=\"5\" align=\"right\"></td></tr>\n");
print("<tr>\n");
	$product1 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='1' ");
if (mysql_num_rows($product1)) {
    $P1 = mysql_fetch_assoc($product1);
    $PQ1=$P1['qty']*1;}
$product2 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='2' ");
if (mysql_num_rows($product2)) {

    $P2 = mysql_fetch_assoc($product2);
    $PQ2=$P2['qty']*1;}
$product3 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='3' ");
if (mysql_num_rows($product3)) {
    $P3 = mysql_fetch_assoc($product3);
    $PQ3=$P3['qty']*1;}
$product4 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='4' ");
if (mysql_num_rows($product4)) {
    $P4 = mysql_fetch_assoc($product4);
    $PQ4=$P4['qty']*1;}
$product5 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='5' ");
if (mysql_num_rows($product5)) {
    $P5 = mysql_fetch_assoc($product5);
    $PQ5=$P5['qty']*6;}
$product6 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='6' ");
if (mysql_num_rows($product6)) {
    $P6 = mysql_fetch_assoc($product6);
    $PQ6=$P6['qty']*15;}
$product7 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='7' ");
if (mysql_num_rows($product7)) {
    $P7 = mysql_fetch_assoc($product7);
    $PQ7=$P7['qty']*1;}
$product8 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='8' ");
if (mysql_num_rows($product8)) {
    $P8 = mysql_fetch_assoc($product8);
    $PQ8=$P8['qty']*1;}
$product9 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='9' ");
if (mysql_num_rows($product9)) {
    $P9 = mysql_fetch_assoc($product9);
    $PQ9=$P9['qty']*1;}
$product10 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='10' ");
if (mysql_num_rows($product10)) {
    $P10 = mysql_fetch_assoc($product10);
    $PQ10=$P10['qty']*1;}
$product11 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='11' ");
if (mysql_num_rows($product11)) {
    $P11 = mysql_fetch_assoc($product11);
    $PQ11=$P11['qty']*1;}
$product12 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='12' ");
if (mysql_num_rows($product12)) {
    $P12 = mysql_fetch_assoc($product12);
    $PQ12=$P12['qty']*6;}	
$product13 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='13' ");
if (mysql_num_rows($product13)) {
    $P13 = mysql_fetch_assoc($product13);
    $PQ13=$P13['qty']*6;}
$product14 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='14' ");
if (mysql_num_rows($product14)) {
    $P14 = mysql_fetch_assoc($product14);
    $PQ14=$P14['qty']*6;}
$product15 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='15' ");
if (mysql_num_rows($product15)) {
    $P15 = mysql_fetch_assoc($product15);
    $PQ15=$P15['qty']*15;}
$product16 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='16' ");
if (mysql_num_rows($product16)) {
    $P16 = mysql_fetch_assoc($product16);
    $PQ16=$P16['qty']*15;}
$product17 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='17' ");
if (mysql_num_rows($product17)) {
    $P17 = mysql_fetch_assoc($product17);
    $PQ17=$P17['qty']*15;}
$product18 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='18' ");
if (mysql_num_rows($product18)) {
    $P18 = mysql_fetch_assoc($product18);
    $PQ18=$P18['qty']*1;}
$product19 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='19' ");
if (mysql_num_rows($product19)) {
    $P19 = mysql_fetch_assoc($product19);
    $PQ19=$P19['qty']*1;}
$product20 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='20' ");
if (mysql_num_rows($product20)) {
    $P20 = mysql_fetch_assoc($product20);
    $PQ20=$P20['qty']*1;}			
$product21 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='21' ");
if (mysql_num_rows($product21)) {
    $P21 = mysql_fetch_assoc($product21);
    $PQ21=$P21['qty']*1;}
	$product22 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='22' ");
if (mysql_num_rows($product22)) {
    $P22 = mysql_fetch_assoc($product22);
    $PQ22=$P22['qty']*1;}
	$product23 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='23' ");
if (mysql_num_rows($product23)) {
    $P23 = mysql_fetch_assoc($product23);
    $PQ23=$P23['qty']*1;}
	$product24 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='24' ");
if (mysql_num_rows($product24)) {
    $P24 = mysql_fetch_assoc($product24);
    $PQ24=$P24['qty']*1;}
	$product25 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='25' ");
if (mysql_num_rows($product25)) {
    $P25 = mysql_fetch_assoc($product25);
    $PQ25=$P25['qty']*1;}
		$product26 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='26' ");
if (mysql_num_rows($product26)) {
    $P26 = mysql_fetch_assoc($product26);
    $PQ26=$P26['qty']*1;}
		$product27 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='27' ");
if (mysql_num_rows($product27)) {
    $P27 = mysql_fetch_assoc($product27);
    $PQ27=$P27['qty']*1;}
		$product28 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='28' ");
if (mysql_num_rows($product28)) {
    $P28 = mysql_fetch_assoc($product28);
    $PQ28=$P28['qty']*1;}
		$product29 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='29' ");
if (mysql_num_rows($product29)) {
    $P29 = mysql_fetch_assoc($product29);
    $PQ29=$P29['qty']*6;}
		$product30 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='30' ");
if (mysql_num_rows($product30)) {
    $P30 = mysql_fetch_assoc($product30);
    $PQ30=$P30['qty']*6;}
		$product31 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='31' ");
if (mysql_num_rows($product31)) {
    $P31 = mysql_fetch_assoc($product31);
    $PQ31=$P31['qty']*6;}
		$product32 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='32' ");
if (mysql_num_rows($product32)) {
    $P32 = mysql_fetch_assoc($product32);
    $PQ32=$P32['qty']*4;}
		$product33 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='33' ");
if (mysql_num_rows($product33)) {
    $P33 = mysql_fetch_assoc($product33);
    $PQ33=$P33['qty']*4;}
		$product34 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='34' ");
if (mysql_num_rows($product34)) {
    $P34 = mysql_fetch_assoc($product34);
    $PQ34=$P34['qty']*4;}
		$product35 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='35' ");
if (mysql_num_rows($product35)) {
    $P35 = mysql_fetch_assoc($product35);
    $PQ35=$P35['qty']*1;}
		$product36 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='36' ");
if (mysql_num_rows($product36)) {
    $P36 = mysql_fetch_assoc($product36);
    $PQ36=$P36['qty']*1;}
		$product37 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='37' ");
if (mysql_num_rows($product37)) {
    $P37 = mysql_fetch_assoc($product37);
    $PQ37=$P37['qty']*1;}
		$product38 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='38' ");
if (mysql_num_rows($product38)) {
    $P38 = mysql_fetch_assoc($product38);
    $PQ38=$P38['qty']*1;}
		$product39 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='39' ");
if (mysql_num_rows($product39)) {
    $P39 = mysql_fetch_assoc($product39);
    $PQ39=$P39['qty']*1;}
		$product40 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='40' ");
if (mysql_num_rows($product40)) {
    $P40 = mysql_fetch_assoc($product40);
    $PQ40=$P40['qty']*6;}
		$product41 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='41' ");
if (mysql_num_rows($product41)) {
    $P41 = mysql_fetch_assoc($product41);
    $PQ41=$P41['qty']*6;}
		$product42 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='42' ");
if (mysql_num_rows($product42)) {
    $P42 = mysql_fetch_assoc($product42);
    $PQ42=$P42['qty']*6;}
		$product43 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='43' ");
if (mysql_num_rows($product43)) {
    $P43 = mysql_fetch_assoc($product43);
    $PQ43=$P43['qty']*6;}
		$product44 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='44' ");
if (mysql_num_rows($product44)) {
    $P44 = mysql_fetch_assoc($product44);
    $PQ44=$P44['qty']*6;}
		$product45 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='45' ");
if (mysql_num_rows($product45)) {
    $P45 = mysql_fetch_assoc($product45);
    $PQ45=$P45['qty']*6;}
		$product46 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='46' ");
if (mysql_num_rows($product46)) {
    $P46 = mysql_fetch_assoc($product46);
    $PQ46=$P46['qty']*6;}
		$product47 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='47' ");
if (mysql_num_rows($product47)) {
    $P47 = mysql_fetch_assoc($product47);
    $PQ47=$P47['qty']*6;}
		$product48 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='48' ");
if (mysql_num_rows($product48)) {
    $P48 = mysql_fetch_assoc($product48);
    $PQ48=$P48['qty']*15;}
		$product49 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='49' ");
if (mysql_num_rows($product49)) {
    $P49 = mysql_fetch_assoc($product49);
    $PQ49=$P49['qty']*15;}
		$product50 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='50' ");
if (mysql_num_rows($product50)) {
    $P50 = mysql_fetch_assoc($product50);
    $PQ50=$P50['qty']*15;}
		$product51 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='51' ");
if (mysql_num_rows($product51)) {
    $P51 = mysql_fetch_assoc($product51);
    $PQ51=$P51['qty']*6;}
		$product52 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='52' ");
if (mysql_num_rows($product52)) {
    $P52 = mysql_fetch_assoc($product52);
    $PQ52=$P52['qty']*6;}
		$product53 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='53' ");
if (mysql_num_rows($product53)) {
    $P53 = mysql_fetch_assoc($product53);
    $PQ53=$P53['qty']*6;}
		$product54 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='54' ");
if (mysql_num_rows($product54)) {
    $P54 = mysql_fetch_assoc($product54);
    $PQ54=$P54['qty']*6;}
		$product55 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='55' ");
if (mysql_num_rows($product55)) {
    $P55 = mysql_fetch_assoc($product55);
    $PQ55=$P55['qty']*6;}
		$product56 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='56' ");
if (mysql_num_rows($product56)) {
    $P56 = mysql_fetch_assoc($product56);
    $PQ56=$P56['qty']*6;}
		$product57 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='57' ");
if (mysql_num_rows($product57)) {
    $P57 = mysql_fetch_assoc($product57);
    $PQ57=$P57['qty']*6;}
		$product58 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='58' ");
if (mysql_num_rows($product58)) {
    $P58 = mysql_fetch_assoc($product58);
    $PQ58=$P58['qty']*6;}
		$product59 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='59' ");
if (mysql_num_rows($product59)) {
    $P59 = mysql_fetch_assoc($product59);
    $PQ59=$P59['qty']*4;}
		$product60 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='60' ");
if (mysql_num_rows($product60)) {
    $P60 = mysql_fetch_assoc($product60);
    $PQ60=$P60['qty']*4;}
		$product61 = mysql_query("SELECT qty FROM tblotsdetail_amigo WHERE otsID = '$o' && itemNo='61' ");
