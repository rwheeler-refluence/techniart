...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_facebook_facebook where sess='$sess' && status='open'";
		$resultc=db_query($sqlc);
		$countc=mysql_num_rows($resultc);
		if(!$countc){
			$sql="insert into tblorderstosend_facebook_facebook values ('','$sess','','','$stamp','open')";
			$result=db_query($sql);
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_facebook_facebook where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_facebook_facebook values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_facebook_facebook set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			$sql="update tblotsdetail_facebook_facebook set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
#print("".$sql."<br />");			
			$result=db_query($sql);
		}
	break;
...
<table width="760" border="0" cellspacing="0" cellpadding="0">
  <tr valign="top">
<td width="760" class="title_bkg"><div id="title_spacer" align="left"><span class="title_main">Order Form </span></div></td>
</tr></table>
...
$sql="select * from tblotsdetail_facebook_facebook where otsID='$o'";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select free_ship from tblProducts_facebook_facebook where productID='$itemNo'";
...
?>
<div align="left"><span class="body_content_style1">*Please note: Sales tax might be added depending on your State. </span></div>
<br>
...
<div>
<table width="760" align="center" cellpadding="1" cellspacing="1">
  <tr valign="top">
...
<td><span class="body_content_style1">Country:</span></td>
<td><input name="ship_country" id="ship_country" type="text" size="25" value="<? echo($country); ?>" class="forms3"></td>
</tr><tr valign="top">
<td colspan="2"><img src="pix/pix_trans.gif" width="1" height="5"></td>
</tr><tr valign="top">
...
<tr valign="top">
  <td colspan="2"><table width="309" height="35" border="0">
    <tr>
      <td width="183"><span class="body_content_style1">Would you like to be emailed with information from Mass Save?</span></td>
      <td width="116"><label></label></td>
    </tr>
  </table></td>
  </tr>
</table>
...
<td><span class="body_content_style1">Country:</span></td>
<td><input name="country" id="country" type="text" size="25" value="<? echo($country); ?>" class="forms3"></td>
</tr></table>
...
<div align="left"><img src="back-bottom.jpg" width="760" height="25">
</div>
</body>
</html>
