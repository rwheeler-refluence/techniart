...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend2 where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail2 where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail2 values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail2 set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail2 where otsdetailID='$otsdetailID[$i]'";
			}else{
...
<script type="text/javascript" src="http://ajax.googleapis.us/ajax/libs/jquery/1.3.2/jquery.min.js"></script>
...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail2 LEFT OUTER JOIN tblProducts on tblotsdetail2.itemNo=tblProducts.productID where tblotsdetail2.otsID='$o'";
$sqlo="select sum(qty) as TotalQuantity from tblotsdetail2 where tblotsdetail2.otsID='$o'";
#print($sql);
#print($sqlo);
$result=db_query($sql);
