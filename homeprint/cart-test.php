...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend_tailgate where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
		$resultc1=db_query($sqlc1);
		$countc1=mysql_num_rows($resultc1);
		if($countc1<1){
			$sql="insert into tblorderstosend_tailgate values ('','$sess','','','$stamp','open')";
#print("c2: ".$sql."<br>");
			$result=db_query($sql);
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail_tailgate where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail_tailgate values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail_tailgate set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail_tailgate where otsdetailID='$otsdetailID[$i]'";
			}else{
				$sql="update tblotsdetail_tailgate set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
			}
			$result=db_query($sql);
		}
	break;
...
<html>
<head>
<title>TechniArt - Eastern Bank Mass Saver Combo Sale</title>
...
<?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail_tailgate LEFT OUTER JOIN tblProducts_tailgate on tblotsdetail_tailgate.itemNo=tblProducts_tailgate.productID where tblotsdetail_tailgate.otsID='$o'";
#print($sql);
$result=db_query($sql);
 ...
 while($row=mysql_fetch_array($result)){
	...
	/*	if(strlen($_SESSION['zip_qualify'])>0){
		...
		if(strlen($_SESSION['zip_qualify'])>0){
			...
			if($pr!==$price){
				$sqlu="update tblotsdetail_tailgate set price='$pr' where otsdetailID='$otsdetailID'";
		#		$resultu=db_query($sqlu);
			}
...
#		print("".$calcweight."<br>");
		...
		$sqlfreeship="select free_ship from tblProducts_tailgate where productID='$productID'";
		...
		print("<td><a class=\"body_content_style1\" href=\"#\" onClick=\"javascript: decision('Are you sure you want to remove this item from your shopping bag?', 'http://www.techniart.com/tailgate/remove.php?ID=".$otsdetailID."');\"><span class=\"body_content_style1\">REMOVE?</span></a></td>\n");
...
#5/21
		#shipping actuals
		$sqlship="select * from tblProducts_tailgate where productID='$itemNo'";
	//	print("".$sqlship."<br>");
...
#	print("</tr>\n");
	...
	print("</tr></form>\n");
	print("<tr>\n");
	print("<td colspan=\"4\" align=\"right\"><a class=\"body_content_style1\" href=\"http://www.techniart.com/tailgate/empty.php?otsID=".$o."\">Empty Cart</a></td>\n");
	print("</tr>\n");
	print("<tr>\n");
	print("<td colspan=\"4\" align=\"right\"><a class=\"body_content_style1\" href=\"http://www.techniart.com/tailgate/index.php\">Continue Shopping</a></td>\n");
	print("</tr>\n");
	print("<tr>\n");
	print("<td colspan=\"4\" align=\"right\"><form method=\"post\" action=\"https://secure40.securewebsession.com/techniart.com/tailgate/orderform.php\"><input type=\"hidden\" name=\"otsID\" value=\"".$o."\"><input type=\"hidden\" name=\"ship\" value=\"".$ship."\"><input type=\"hidden\" name=\"state\" value=\"".$_SESSION["shipping"]["state"]."\"><input type=\"submit\" value=\"Checkout\"></td>\n");
	print("</tr></form>\n");
	print("</table></form>\n");
}	?>
<br>
