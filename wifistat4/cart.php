...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend4 where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail4 where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail4 values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail4 set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail4 where otsdetailID='$otsdetailID[$i]'";
			}else{
...
<link href="https://www.techniart.us/wifistat3/boilerplate.css" rel="stylesheet" type="text/css">
<link href="https://www.techniart.us/wifistat3/mobile.css" rel="stylesheet" type="text/css">
<!-- 
...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail4 LEFT OUTER JOIN tblProducts on tblotsdetail4.itemNo=tblProducts.productID where tblotsdetail4.otsID='$o'";
$sqlo="select sum(qty) as TotalQuantity from tblotsdetail4 where tblotsdetail4.otsID='$o'";
#print($sql);
#print($sqlo);
$result=db_query($sql);
...
#	print("</tr>\n");
	...
	print("</tr></form>\n");
	print("<tr>\n");
	print("<td colspan=\"5\" align=\"right\"><input type=\"button\" class=\"btn1\" value=\"Empty Cart\" onclick=\"location.href='http://www.techniart.us/wifistat3/empty.php?otsID=".$o."'\"></td>\n");
	print("</tr>\n");
