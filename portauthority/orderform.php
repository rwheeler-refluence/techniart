...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_illumina where sess='$sess' && status='open'";
		$resultc=db_query($sqlc);
		$countc=mysql_num_rows($resultc);
		if(!$countc){
			$sql="insert into tblorderstosend_illumina values ('','$sess','','','$stamp','open')";
			$result=db_query($sql);
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_illumina where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_illumina values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_illumina set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			$sql="update tblotsdetail_illumina set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
#print("".$sql."<br />");			
			$result=db_query($sql);
		}
	break;
...
$sql="select * from tblotsdetail_illumina where otsID='$o'";
$result=db_query($sql);
...
THIS PROMOTION IS FOR SAN DIEGO GAS & ELECTRIC RESIDENTIAL CUSTOMERS ONLY!<br>
  <br>
</div>
<div class="accordion">
    <div class="accordion-section">
        ...
        <div class="accordion">
    <div class="accordion-section">
        <a class="accordion-section-title" href="#accordion-2">Contact Information - Click to slide open</a>
