...
if (mysql_num_rows($product5)) {
	...
	$product242 = mysql_query("SELECT qty FROM tblotsdetail WHERE otsID = '$o' && itemNo='242' ");
if (mysql_num_rows($product242)) {
    $P242 = mysql_fetch_assoc($product242);
    $a19limit=$P242['qty']*1;}

	$product243 = mysql_query("SELECT qty FROM tblotsdetail WHERE otsID = '$o' && itemNo='243' ");
if (mysql_num_rows($product243)) {
    $P243 = mysql_fetch_assoc($product243);
    $br30limit=$P243['qty']*1;}
	
	$lamplimit=$a19limit+$br30limit;
	
	print("<tr>\n");
	print("<td colspan=\"4\" align=\"right\"><input type=\"button\" value=\"Continue Shopping\" class=\"btn\" onclick=\"location.href='http://www.techniart.us/sample/store.php'\"></td>\n");
	print("</tr>\n");
	print("<tr height=\"3\"><td colspan=\"3\" align=\"right\"></td></tr>");
	if($bulblimit > '12'){
 ...
 or less</b>.</td>\n");	
	...
	print("<tr>\n");
	print("<td colspan=\"4\" align=\"right\"><form method=\"post\" action=\"https://www.techniart.us/sample/orderform.php\"><input type=\"hidden\" name=\"otsID\" value=\"".$o."\"><input type=\"submit\" class=\"btn\" value=\"Checkout\"></td>\n");
	print("</tr></form>\n");
	print("</table></form>\n");
	}}}}
	if($bulblimit<1){
