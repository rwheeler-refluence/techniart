...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_$rep where sess='$sess' && status='open'";
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_$rep where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_$rep values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_$rep set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
...
$sql="select * from tblotsdetail_$rep where otsID='$o'";
$result=db_query($sql);
