...
$instr=$_POST['instr'];
$order=$ship_fname . ',' . $ship_lname . ',' . $ship_address1 . ',' . $ship_address2 . ',' . $ship_city . ',' . $ship_state . ',' . $ship_zip . ',' . $fname . ',' . $lname . ',' . $address1 . ',' . $address2 . ',' . $city . ',' . $state . ',' . $zip . ',' . $pledge . ',' . $customer . ',' . $swear . ',' . $phone . ',' . $email;
?>
<?
$ship_zip=$_POST['ship_zip'];
...
if($countzip<1){echo "<SCRIPT LANGUAGE='JavaScript'>
	window.alert('The shipping zip code you entered does not qualify!')
		window.location.href='https://www.techniart.us/led/orderform.php'
        </SCRIPT>
        ";
		die();}
