...
if($ID){
	$act=$_GET['act'];
	if($act=='pub'){
		$sql="update tblProducts_raytheon set productPub='Yes' where productID='$ID'";
	}else{
		$sql="update tblProducts_raytheon set productPub='No' where productID='$ID'";
	}
	$result=db_query($sql);
...
print("<br>\n");
		...
		$sql="select distinct tblProducts_raytheon.category from tblProducts_raytheon LEFT OUTER JOIN tblSort on tblProducts_raytheon.category=tblSort.category order by tblSort.sort asc";
		$result=db_query($sql);
