...
$i=1;
	...
	print("</b></span><br>");
	if(!$subcat){
		$sqlsubs="select distinct subcat1 from tblProducts_pse where category='$cat' && subcat1!='' order by $sort asc";
		...
		$sqlsubs="select distinct subcat2 from tblProducts_pse where subcat1='$subcat' && category='$cat' && subcat2!='' order by subcat2 asc";
		$resultsubs=db_query($sqlsubs);
		$countsubs=mysql_num_rows($resultsubs);
		$subs=1;
		while($rowsubs=mysql_fetch_array($resultsubs)){
...
while($rowp=mysql_fetch_array($resultp)){
	...
	$modelNumber=$rowp['modelNumber'];
			$loc=$rootDir."microsoft/pix/products/thumbnails/".$imageLoc;
