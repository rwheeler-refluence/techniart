...
$sql="select * from tblotsdetail_pse where otsID='$o' ORDER BY ItemNo + 0 ASC";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select modelNumber from tblProducts_pse where productID='$itemNo'";
		...
		if(!$_SESSION['zip']){
			$zz=$_POST['ship_zip'];
			$sqlz="select * from tblDiscounts LEFT OUTER JOIN tblProducts_pse on tblDiscounts.item_no=tblProducts_pse.modelNumber where tblProducts_pse.productID='$itemNo' && zip='$zz'";
			#print($sqlz);
			...
			if($countz>0){
				while($rowz=mysql_fetch_array($resultz)){
					...
					$sqlu="update tblotsdetail_pse set price='$disct_price' where otsdetailID='$otsdetailID'";
					#print($sqlu);
		...
		print("<td valign=\"middle\"><span class=\"body_content_style1\">".$productDesc."".$lbl."</span></td>\n");
		print("<td valign=\"middle\"><span class=\"body_content_style1\">$".number_format($price, 2, '.', ',')."</span></td>\n");
		print("<td valign=\"middle\"><span class=\"body_content_style1\">$".number_format($tot, 2, '.', ',')."</span></td>\n");
		print("</tr>\n");
	}
...
$rate = '0.00';
#$sqltest = "SELECT ItemNo FROM tblotsdetail_pse WHERE otsID='$o' ORDER BY ItemNo + 0 ASC";
#$sqltest = "SELECT ItemNo FROM tblotsdetail_pse WHERE otsID='$o' ORDER BY ItemNo ASC";
...
#	print("<td colspan=\"5\" align=\"right\">");
		if($state=='CT' || $state=='RI' || $state=='WA' || $state=='MA' || $state=='CA'){
		$sales_tax_tot=$carttot;
		switch($state){
			case "CT":
				$taxamt=$sales_tax_tot*.10;
			break;
			case "RI":
