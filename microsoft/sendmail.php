...
if($security!==md5($security_array[$chosen])){
	header("location: contact.php?msg=security");
}else{
	...
	$headers .= "Reply-To: " . stripslashes(checkit($_POST['email'])) . "\r\n";
		...
		$subject="Submission from techniart Web Site";
		...
		$body.="Email: $email\n";
		$body.="Phone: $phone\n";
		$body.="Best Time: $best_time\n";
		$body.="\n\n";
		$body.="" . stripslashes(checkit($_POST['notes'])) . "";
	//	mail($To,$subject,$body,"From: $email");
		@mail($To, $subject, $body, $headers);
		$stamp=mktime();
		$sql="insert into tblContact values ('','$fname','$lname','$address','$address2','$city','$state','$zip','$email','$phone','$best_time','$interested','$dvd','$stamp')";
		$result=db_query($sql);
		echo "<SCRIPT LANGUAGE='JavaScript'>
        window.alert('Your bulb swap has been regsitered.')
