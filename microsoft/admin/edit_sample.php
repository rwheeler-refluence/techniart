...
if($action=='add'){
	...
	$presID=$_POST['presID'];
	$sql.="update tblPres set presTitle='$presTitle'";
	...
	if($_FILES['presFile']['tmp_name']!==''){
		...
		$sql.=",presFile='$imgpath'";
	}
	$sql.="	, presType='$presType' where presID='$presID'";
	#print($sql);
	$result=db_query($sql);
...
<?
$ID=$_GET['ID'];
$sql="select * from tblPres where presID='$ID'";
$result=db_query($sql);
while($row=mysql_fetch_array($result)){
	$presID=$row['presID'];
	$presFile=stripslashes($row['presFile']);
...
<td><span class="title_main_sub">Presentation Samples:</span>&nbsp;&nbsp;<span class="title_sub_level2">Edit Presentation Sample</span><br></td>
<td align="right">&nbsp;</td>
</tr><tr>
...
<input type="hidden" name="presID" value="<? echo($presID); ?>">
...
<br>
<img src="pix/pix_trans.gif" alt="" width="1" height="8" border="0"><br>
<input type="submit" name="Submit" value="Update Presentation Sample"></form>
<br>
