...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend1 where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail1 where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail1 values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail1 set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail1 where otsdetailID='$otsdetailID[$i]'";
			}else{
...
<link href="https://www.techniart.us/wifistat1/boilerplate.css" rel="stylesheet" type="text/css">
<link href="https://www.techniart.us/wifistat1/mobile.css" rel="stylesheet" type="text/css">
<!-- 
...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail1 LEFT OUTER JOIN tblProducts on tblotsdetail1.itemNo=tblProducts.productID where tblotsdetail1.otsID='$o'";
$sqlo="select sum(qty) as TotalQuantity from tblotsdetail1 where tblotsdetail1.otsID='$o'";
#print($sql);
#print($sqlo);
$result=db_query($sql);
...
print("<form  method=\"post\" action=\"".$PHP_SELF."\">\n");
	...
	while($row=mysql_fetch_array($result)){
		...
		print("<td align=\"center\"><input type=\"button\" class=\"btn1\" value=\"Update\" onClick=\"this.form.submit();\"></td>\n");
...
#	print("</tr>\n");
	...
	print("</tr></form>\n");
	print("<tr>\n");
	print("<td colspan=\"5\" align=\"right\"><input type=\"button\" class=\"btn1\" value=\"Empty Cart\" onclick=\"location.href='http://www.techniart.us/wifistat1/empty.php?otsID=".$o."'\"></td>\n");
	print("</tr>\n");
	
	print("<tr>\n");
	if($limit > '2'){
	print("<td colspan=\"5\" align=\"right\" class=\"style5\">In order to checkout,<br>please adjust your cart<br>to <b>2 total units</b>.</td>\n");	
	print("</tr>\n");
	print("<tr>\n");
	print("</table>\n");
	}else{
	print("<td colspan=\"5\" align=\"right\"><form method=\"post\" action=\"orderform.php\"><input type=\"hidden\" name=\"otsID\" value=\"".$o."\"><input type=\"hidden\" name=\"ship\" value=\"".$ship."\"><input type=\"hidden\" name=\"state\" value=\"".$_SESSION["shipping"]["state"]."\"><input type=\"submit\" class=\"btn1\" value=\"Checkout\"></td>\n");
	print("</tr></form>\n");
	print("<tr>\n");
	print("<td colspan=\"5\" align=\"right\"></td>\n");
	print("</tr>\n");
