...
$sqlp.="select * from tblProducts_smeco where category='$cat' ";
if($subcat){
$sqlp.="&& subcat1='$subcat'";
	}
	if($subcat1){
...
$i=1;
	...
	print("</b></span><br>");
	if(!$subcat){
		$sqlsubs="select distinct subcat1 from tblProducts_smeco where category='$cat' && subcat1!='' order by $sort asc";
		...
		$sqlsubs="select distinct subcat2 from tblProducts_smeco where subcat1='$subcat' && category='$cat' && subcat2!='' order by subcat2 asc";
		$resultsubs=db_query($sqlsubs);
		$countsubs=mysql_num_rows($resultsubs);
		$subs=1;
		while($rowsubs=mysql_fetch_array($resultsubs)){
...
while($rowp=mysql_fetch_array($resultp)){
	...
	$modelNumber=$rowp['modelNumber'];
			$loc=$rootDir."SMECO/pix/products/thumbnails/".$imageLoc;
