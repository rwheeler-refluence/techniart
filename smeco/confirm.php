...
$sql="select * from tblotsdetail_smeco where otsID='$o' ORDER BY ItemNo + 0 ASC";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		if(!$_SESSION['zip']){
			$zz=$_POST['ship_zip'];
			$sqlz="select * from tblDiscounts LEFT OUTER JOIN tblProducts_smeco on tblDiscounts.item_no=tblProducts_smeco.modelNumber where tblProducts_smeco.productID='$itemNo' && zip='$zz'";
			#print($sqlz);
			...
			if($countz>0){
				while($rowz=mysql_fetch_array($resultz)){
					...
					$sqlu="update tblotsdetail_smeco set price='$disct_price' where otsdetailID='$otsdetailID'";
					#print($sqlu);
...
$rate = '0.00';
#$sqltest = "SELECT ItemNo FROM tblotsdetail_smeco WHERE otsID='$o' ORDER BY ItemNo + 0 ASC";
#$sqltest = "SELECT ItemNo FROM tblotsdetail_smeco WHERE otsID='$o' ORDER BY ItemNo ASC";
...
#	print("<td colspan=\"5\" align=\"right\">");
		if($state=='MD' || $state=='RI' || $state=='WA' || $state=='MA' || $state=='CA'){
...
<br>
<b>Office location:</b> <? echo($pickup); ?>
</span></td>
</tr></table>
</span></td>
<td width="10">&nbsp;</span></td>
