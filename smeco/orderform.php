...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_smeco where sess='$sess' && status='open'";
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_smeco where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_smeco values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_smeco set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
...
$sql="select * from tblotsdetail_smeco where otsID='$o'";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select modelNumber from tblProducts_smeco where productID='$itemNo'";
...
if (mysql_num_rows($product3)) {
    ...
    $PQ4=$P4['qty']*6;}
	$product5 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='5' ");
if (mysql_num_rows($product5)) {
    $P7 = mysql_fetch_assoc($product5);
    $PQ5=$P5['qty']*6;}
	$product6 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='6' ");
	if (mysql_num_rows($product6)) {
    $P6 = mysql_fetch_assoc($product6);
    $PQ6=$P6['qty']*1;}
	$product7 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='7' ");
	if (mysql_num_rows($product7)) {
    $P7 = mysql_fetch_assoc($product7);
    $PQ7=$P7['qty']*6;}
	$product8 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='8' ");
	if (mysql_num_rows($product8)) {
    $P8 = mysql_fetch_assoc($product8);
    $PQ8=$P8['qty']*1;}
	$product9 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='9' ");
	if (mysql_num_rows($product9)) {
    $P9 = mysql_fetch_assoc($product9);
    $PQ9=$P9['qty']*1;}
	$product10 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='10' ");
	if (mysql_num_rows($product10)) {
    $P10 = mysql_fetch_assoc($product10);
    $PQ10=$P10['qty']*1;}
	$bulblimit=$PQ1+$PQ2+$PQ3+$PQ4+$PQ5+$PQ6+$PQ7+$PQ8+$PQ9+$PQ10;
	$product13 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='13' ");
	if (mysql_num_rows($product13)) {
    $P13 = mysql_fetch_assoc($product13);
    $PQ13=$P13['qty']*1;}
	$product14 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='14' ");
	if (mysql_num_rows($product14)) {
    $P14 = mysql_fetch_assoc($product14);
    $PQ14=$P14['qty']*1;}
	$product15 = mysql_query("SELECT qty FROM tblotsdetail_smeco WHERE otsID = '$o' && itemNo='15' ");
...
<tr valign="top">
            ...
            <td align="left"><select name="pickup" class="forms10">
              <option></option>
              <option value="SMECO HQ - Hughsville (9a-12p)">SMECO HQ - Hughsville (9a-12p)</option>
              <option value="Leonardtown Office - Leonardtown (2p-4p)">Leonardtown Office - Leonardtown (2p-4p)</option>
            </select></td>
          </tr>
