...
if($vendorName){
	$sql="select * from tblOrdersCompleted where tblOrdersCompleted.stamp BETWEEN '$start' AND '$end' && tblOrdersCompleted.access_company='$vendorName' order by tblOrdersCompleted.completeID desc";
#	print($sql);
	$result=db_query($sql);
	$count=mysql_num_rows($result);
	if(!$count){
	}else{
		$out.="\"Date\",\"Name\",\"Address\",\"Address 2\",\"City\",\"State\",\"Zip\",\"Received?\",\"Tracking\",\"A19\",\"Globe\",\"BR30\",\"Bath Aerator\",\"Kitchen Aerator\",\"Showerhead\",\"Handheld\",\n";
...
$BR20limit='0';}
			...
			$out.="\"".$timestamp."\",\"".$ship_firstname." ".$ship_lastname."\",\"".$ship_address."\",\"".$ship_address2."\",\"".$ship_city."\",\"".$ship_state."\",\"".$ship_zip."\",\"".$status."\",\"".$tracking."\",\"".$ENLlimit."\",\"".$DNLlimit."\",\"".$Candlelimit."\",\"".$Globelimit."\",\"".$MR16limit."\",\"".$Threewaylimit."\",\"".$BR20limit."\",\n";
		}
}
