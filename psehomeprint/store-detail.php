...
<td width="170"><a rel="shadowbox;" href="<? echo($loc);?>" class="option" title="<? echo($productName);?>"><img src="<? echo($loc);?>" alt="<? echo($productName);?>" width="200px" height="<? echo($height);?>" border="0" class="img_stroke"></a><br>
<? 
if($energy_star=='Yes'){
...
$case_price=$disct_price*$case_quantity;
?>
...
<?}?>-->
<span class="product_price">$<? echo(number_format($disct_price, 2, '.', ','));?> per unit</span><br>
<span class="product_price">$<? echo(number_format($case_price, 2, '.', ','));?> per case</span><br>
<br>
...
<?}?>
<? if($case_quantity){?>
<b>Case Quantity:</b> <? echo($case_quantity);?><br>
<?}?>
<? if($manuf){?>
...
<input type="button" class="btn1" value="Add to Cart" onClick="this.form.submit();">
<?}?>&nbsp;&nbsp;<input type="button" class="btn3" value="Go Back" onclick='location.href="javascript:history.go(-1)";'/>
<br></span></td>
<? if($class=="light"){?>
<td width="300" valign="top"><table align="center" width="255" border="2" bordercolor="#000000">
  <tr>
