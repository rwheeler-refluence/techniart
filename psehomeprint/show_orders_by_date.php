...
print("<form method=\"GET\" action=\"".$_SERVER['PHP_SELF']."\"><select name=\"vendorName\" size=\"1\">\n");
print("<option value=\"\">");
$sql="select * from tblVendors order by vendorName asc";
$result=db_query($sql);
while($row=mysql_fetch_array($result)){
	$vendorName1=$row['vendorName'];
	$vendorName=$row['vendorName'];
	print("<option value=\"".$vendorName1."\"");
	if($vendorName1==$vendorName){
		print(" selected");
	}
	print(">".$vendorName."\n");
}
...
if($vendorName){
	print("<br>");
	$sql="select * from tblOrdersCompleted where tblOrdersCompleted.stamp BETWEEN '$start' AND '$end' && tblOrdersCompleted.email_opt='$vendorName' order by tblOrdersCompleted.completeID desc";
#	print($sql);
	$result=db_query($sql);
	...
	if(!$count){
		...
		print("<div align=\"right\" style=\"padding-right:20px;\"><a class=\"body_content\" href=\"dl_orders_by_vendor.php?vendorName=".$vendorName."&start=".$_GET['start']."&end=".$_GET['end']."\">Download as *.csv</a></div>\n");
