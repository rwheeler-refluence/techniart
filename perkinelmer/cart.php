...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend_raytheon where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
		$resultc1=db_query($sqlc1);
		$countc1=mysql_num_rows($resultc1);
		if($countc1<1){
			$sql="insert into tblorderstosend_raytheon values ('','$sess','','','$stamp','open')";
#print("c2: ".$sql."<br>");
			$result=db_query($sql);
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail_raytheon where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail_raytheon values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail_raytheon set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail_raytheon where otsdetailID='$otsdetailID[$i]'";
			}else{
				$sql="update tblotsdetail_raytheon set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
			}
			$result=db_query($sql);
		}
	break;
...
<?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail_raytheon LEFT OUTER JOIN tblProducts_raytheon on tblotsdetail_raytheon.itemNo=tblProducts_raytheon.productID where tblotsdetail_raytheon.otsID='$o'";
#print($sql);
$result=db_query($sql);
...
if(!$count){
 ...
 while($row=mysql_fetch_array($result)){
	...
	/*	if(strlen($_SESSION['zip_qualify'])>0){
			$pr=$row['disct_price'];
			if($pr!==$price){
			$sqlu="update tblotsdetail_raytheon set price='$pr' where otsdetailID='$otsdetailID'";
...
#		print("".$calcweight."<br>");
		...
		$sqlfreeship="select free_ship from tblProducts_raytheon where productID='$productID'";
...
#5/21
		#shipping actuals
		$sqlship="select * from tblProducts_raytheon where productID='$itemNo'";
	//	print("".$sqlship."<br>");
...
#	print("<tr>\n");
	...
	print("</tr>\n");
	print("<tr height=\"3\"><td colspan=\"3\" align=\"right\"></td></tr>");
	$product1 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='1' ");
if (mysql_num_rows($product1)) {
    $P1 = mysql_fetch_assoc($product1);
    $PQ1=$P1['qty']*1;}
$product2 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='2' ");
if (mysql_num_rows($product2)) {
    $P2 = mysql_fetch_assoc($product2);
    $PQ2=$P2['qty']*1;}
$product3 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='3' ");
if (mysql_num_rows($product3)) {
    $P3 = mysql_fetch_assoc($product3);
    $PQ3=$P3['qty']*1;}
	$product4 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='4' ");
	if (mysql_num_rows($product4)) {
    $P4 = mysql_fetch_assoc($product4);
    $PQ4=$P4['qty']*6;}
	$product5 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='5' ");
if (mysql_num_rows($product5)) {
    $P7 = mysql_fetch_assoc($product5);
    $PQ5=$P5['qty']*6;}
	$product6 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='6' ");
	if (mysql_num_rows($product6)) {
    $P6 = mysql_fetch_assoc($product6);
    $PQ6=$P6['qty']*1;}
	$product7 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='7' ");
	if (mysql_num_rows($product7)) {
    $P7 = mysql_fetch_assoc($product7);
    $PQ7=$P7['qty']*6;}
	$product8 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='8' ");
	if (mysql_num_rows($product8)) {
    $P8 = mysql_fetch_assoc($product8);
    $PQ8=$P8['qty']*1;}
	$product9 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='9' ");
	if (mysql_num_rows($product9)) {
    $P9 = mysql_fetch_assoc($product9);
    $PQ9=$P9['qty']*1;}
	$product10 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='10' ");
	if (mysql_num_rows($product10)) {
    $P10 = mysql_fetch_assoc($product10);
    $PQ10=$P10['qty']*1;}
		$product569 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='569' ");
	if (mysql_num_rows($product569)) {
    $P569 = mysql_fetch_assoc($product569);
    $PQ569=$P569['qty']*1;}
		$product571 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='571' ");
	if (mysql_num_rows($product571)) {
    $P571 = mysql_fetch_assoc($product571);
    $PQ571=$P571['qty']*1;}
		$product568 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='568' ");
	if (mysql_num_rows($product568)) {
    $P568 = mysql_fetch_assoc($product568);
    $PQ568=$P568['qty']*6;}
		$product572 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='572' ");
	if (mysql_num_rows($product572)) {
    $P572 = mysql_fetch_assoc($product572);
    $PQ572=$P572['qty']*1;}
		$product573 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='573' ");
	if (mysql_num_rows($product573)) {
    $P573 = mysql_fetch_assoc($product573);
    $PQ573=$P573['qty']*1;}
	$bulblimit=$PQ1+$PQ2+$PQ3+$PQ4+$PQ5+$PQ6+$PQ7+$PQ8+$PQ9+$PQ10+$PQ568+$PQ569+$PQ571+$PQ572+$PQ573;
	$product13 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='13' ");
	if (mysql_num_rows($product13)) {
    $P13 = mysql_fetch_assoc($product13);
    $PQ13=$P13['qty']*1;}
	$product14 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='14' ");
	if (mysql_num_rows($product14)) {
    $P14 = mysql_fetch_assoc($product14);
    $PQ14=$P14['qty']*1;}
	$product15 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='15' ");
	...
	$APSlimit=$PQ13+$PQ14+$PQ15;
		$product242 = mysql_query("SELECT qty FROM tblotsdetail_raytheon WHERE otsID = '$o' && itemNo='242' ");
if (mysql_num_rows($product242)) {
	...
	print("<tr height=\"3\"><td colspan=\"3\" align=\"right\"></td></tr>");
	if($bulblimit > '24'){
	print("<td colspan=\"4\" align=\"right\" class=\"style5\">In order to checkout,<br>please adjust your cart<br>to <b>24 total bulbs or less</b>.</td>\n");	
	print("</tr>\n");
	print("<tr>\n");
	print("</table>\n");
	}else{
 ...
 or less</b>.</td>\n");	
	...
	if($bulblimit<1){
		$bulblimit='0';}
	if($bulblimit>0){
	print("<span class=\"style5\"><br>You currently have <strong>".$bulblimit."</strong> of the allowed <strong>24</strong> bulbs in your cart.</span><br>
\n");}
	if($bulblimit > '24'){
		print("<span class=\"body_content_ip\">Please adjust your cart so that you have <strong>24</strong> total bulbs or less.</span><br>
<br>
");}
	if($APSlimit<1){
		$APSlimit='0';}
...
\n");}
	if($APSlimit > '5'){
		...
		$lamplimit='0';}
if($lamplimit>0){		
print("<span class=\"style5\"><br>You currently have <strong>".$lamplimit."</strong> of the allowed <strong>4</strong> Combos in your cart.<br>
\n");}
	if($lamplimit > '4'){
