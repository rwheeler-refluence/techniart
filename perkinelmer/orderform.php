...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_raytheon where sess='$sess' && status='open'";
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_raytheon where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_raytheon values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_raytheon set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
...
$sql="select * from tblotsdetail_raytheon where otsID='$o'";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select modelNumber from tblProducts_raytheon where productID='$itemNo'";
...
<div class="accordion">
    ...
    <div class="accordion-section">
        ...
        <table width="739" align="left" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr valign="top">
            <td align="left"><span class="product_title_sm">Please select order pick-up location</span></td>
            <td align="left"><select name="pickup" class="forms10">
              <option></option>
