                ...
                if (x==0) {
	              ...
	              { 
                   ...
                   if (r1[29] > 0){
                     ...
                     document.getElementById('taxratedisplay').innerHTML="Total Tax rate is "+taxDisplay+"%";
                   } 
...
function binvbuild(){
 ...
 //this will keep taxes data driven- just add zip and a zip4 range and state local & tot rate when disticts are added
  ...
  if (trim(document.getElementById('binv_taxflag').value) =="Y" && trim(document.getElementById('binv_shipzip4').value)=='' && trim(document.getElementById('binv_shipst').value)=='WA'){
    hidewait();
    document.body.style.cursor='auto';
  ...
  s[3] = trim(document.getElementById('binv_shipst').value);
...
function binvbuildResponse(){
  ...
  if (http.readyState == 4) {
	...
	if (tempst=="TX"){	
  	...
  	//alert(document.getElementById('binv_taxLocal').value+" | "+document.getElementById('binv_taxState').value+" | "+document.getElementById('binv_taxTotal').value+" | "+document.getElementById('binv_taxCode').value); 
...
function calctotals(){
  ...
  // now actaully change everything to 'P' if anything is taxable - important that branches do not mix non tax and taxable invloices
...
if (trim(mtaxable)=="Y"){
  ...
  if (trim(document.getElementById('binv_shipst').value)=="FL"){
    
    var fl_loctax=parseFloat(document.getElementById('binv_taxLocal').value);
    var fl_statetax=parseFloat(document.getElementById('binv_taxState').value);	
    var temptotal=0;
    
    xf=0;
    var mcntytaxable=0;  
   	while (xf < document.forms['invoiceform'].binv_lnitems.options.length){
        
	    tempVar=document.forms['invoiceform'].binv_lnitems[xf].value;
  	    s= new Array(); 
	  	s= tempVar.split("|");
	  	
	  	//alert(s[0]+" - "+s[1]+" - "+s[2]+" - "+s[3]+" - "+s[4]+" - "+s[5]+" - "+s[6]);
	  	temptotal=temptotal+s[0];
	  	var aPos1 = s[1].indexOf("E-MAIL DATA");
	  	var aPos2 = s[1].indexOf("FTP DATA");
	  	var aPos3 = s[1].indexOf("EDT CHARGES");
	  	if (aPos1 < 0 && aPos2 < 0 && aPos3 < 0){
		  if (s[6]=="P"){
    	     //document.forms['invoiceform'].binv_lnitems[xf].text=document.forms['invoiceform'].binv_lnitems[xf].text+" Y";	
   		     mcntytaxable=mcntytaxable+parseFloat(s[0]);
		  }   
        }    
  	...
  	//tax all of the taxable items at state rate
  	var sttax=(mcntytaxable * fl_statetax);
  	
  	//lop off that beyond 5 grand
  	if (mcntytaxable > 5000){
	   mcntytaxable=5000;
    }  	
  	  
    var ltax=(mcntytaxable * fl_loctax);
    
		   ltaxDisplay=fl_loctax;
		   ltaxDisplay=(ltaxDisplay*100);
		   ltaxDisplay=ltaxDisplay.toFixed(2);
		   
		   sttaxDisplay=fl_statetax;
		   sttaxDisplay=(sttaxDisplay*100);
		   sttaxDisplay=sttaxDisplay.toFixed(2);
		   
		   //taxDisplay=taxDisplay+'';
           document.getElementById('taxratedisplay').innerHTML="State: "+sttaxDisplay+" % Cnty:"+ltaxDisplay+" %";
    
     mtax=(sttax+ltax);
  	
  } else {    	
  	
	//do tax ---- this has been like this for years, not sure why as it clearly is overritten by the next section
  	if (taxitall=="YES"){
		mtax=(msub*taxRate);		   
  	}
...
if (document.forms['invoiceform'].binv_lnitems.options.length <= 0){
	  errmsg=errmsg+"You must enter at least one invoice detail line item.";
} 	  
...
var afind = tempVar.indexOf("NO SHIPPING");
	...
	if (afind !=-1){
        ...
        errmsg=errmsg+"<br><br>You must choose a shipping method.";
 ...
 if (http.readyState == 4) {
    ...
    if (returnText =="Invoice has been saved"){
	   document.getElementById('tk_hasinvoice').value ="Y";
	    ...
	    var tempinvdt=trim(document.getElementById('binv_date_done').value);
        var aPosition = tempinvdt.indexOf("/");
        var secondPos = tempinvdt.indexOf("/", aPosition + 1);
        iMonth=tempinvdt.substring(0,aPosition); 
        iYear=tempinvdt.substring(secondPos+1);	
	    ...
	    //document.getElementById('tk_stkDATE_DONE').value =newdt;
	    
	    var tempinvdt=trim(document.getElementById('binv_invdue').value);
        var aPosition = tempinvdt.indexOf("/");
        var secondPos = tempinvdt.indexOf("/", aPosition + 1);
        iMonth=tempinvdt.substring(0,aPosition); 
        iYear=tempinvdt.substring(secondPos+1);	
	    ...
	    //document.getElementById('tk_stkDATE_DUE').value =newdt;
	    
	    //document.getElementById('tk_stkCONTACT').value = trim(document.getElementById('binv_acctcont').value);
	    document.getElementById('tk_stkPO').value = trim(document.getElementById('binv_po').value);
    ...
    if(detailVal=="Error saving invoice, please try again."){
	    ...
	    if(detailVal == document.forms['invoiceform'].binv_lnitems.length){
	       ...
	       if (trim(document.getElementById('udept').value)=="D"){
		      //alert("In dallas"); 	       
              printcisInvoice();
           } else if (trim(document.getElementById('udept').value)=="DS"){
		      //alert("In dallas"); 	       
              printcisInvoice();
           } else {
	          acctngInvoice(trim(document.getElementById('binv_JOB_ID').value));
