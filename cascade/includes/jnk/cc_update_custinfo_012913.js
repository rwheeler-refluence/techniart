...
if (checkforErrors == 0) {
  ...
  //end of test   
   ...
   //if (document.getElementById('flash_edbox').checked == true && document.getElementById('flashedscr').style.visibility =='visible') {
      //s[145]= "Y";
   //} 
   s[145]=trim(document.getElementById('flash_ed').value);
   ...
   //status drop down
   msel=document.forms['custcareform'].thestatus.selectedIndex;        
   newstat=document.forms['custcareform'].thestatus.options[msel].value
   ...
   var schemaValue = document.forms['custcareform'].schemadefine.options[mindex].value;
  
   s[162]=schemaValue;
   
   if (document.getElementById('DSOAbox').checked == false) {
      s[163]= "N";
   } else {s[163]= "Y"};  
   
   
   s[164]=trim(document.getElementById('flash_ed_sav').value);
  ...
  s[165] = document.getElementById('PAFNUM2').value;

  // s[51] is a date and needs date conversion
  tdt=document.getElementById('PAFEXP2').value;
  if (tdt.length > 6){
     ...
     if (tgtemp[0].length < 2){tgtemp[0]='0' + tgtemp[0]};
     if (tgtemp[1].length < 2){tgtemp[1]='0' + tgtemp[1]};
     s[166]=tgtemp[2]+tgtemp[0]+tgtemp[1];   
	  
  } else {s[166]= " "};

 
  //alert("PAFNUM2: "+s[165]+" PAFEXP2: "+s[166]);
...
//numerrors=numerrors+checkemail('SHIPNOEMA1','shipping Email');
  ...
  numerrors=numerrors+checkdatefmt('PAFEXP2','PAF expiration ');
