...
<br>
<br><div align="center">
	  <img src="landing-graphic.jpg"></div>
...
$instr=$_POST['instr'];
	...
	$sql="insert into tblOrdersCompleted_partners(stamp, fname, lname, streetnum, route, unit1, city, state, zip, zip4, email, ship_fname, ship_lname, ship_streetnum, ship_route, ship_unit1, ship_city, ship_state, ship_zip, ship_zip4, pledge, customer, swear, code, source)	values('$date','$fname','$lname', '$streetnum', '$route', '$unit1', '$city', '$state', '$zip', '$zip4', '$email', '$ship_fname', '$ship_lname', '$ship_streetnum', '$ship_route', '$ship_unit1', '$ship_city', '$ship_state','$ship_zip', '$ship_zip4','$pledge','$customer','$swear', '$code', '$source')";
	$result=db_query($sql);
...
#grab unnotified orders
	$sqla="select * from tblOrdersCompleted_partners where completeID='$next'";
...
//			$body.="Sent on: ".date("m/d/Y H:i:s")."\n\n";
			...
			$sql="select * from tblotsdetail_partners where otsID='$otsID' order by otsdetailID desc";
			#print($sql);
			$result=db_query($sql);
...
function Redirect()
{
    window.location="http://www.techniart.us/sdgeenergysolutionspartners/thanks.php";
}
