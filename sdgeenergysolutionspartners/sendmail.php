...
if($security!==md5($security_array[$chosen])){
	header("location: support.php?msg=security");
}else{
	...
	$headers .= "Reply-To: " . stripslashes(checkit($_POST['email'])) . "\r\n";
		...
		$subject="Submission from SDG&E Water & Energy Kit Portal - techniart.us/sdgeenergysolutionspartners";
	...
	//	mail($To,$subject,$body,"From: $email");
		...
		$stamp=mktime();
		$sql="insert into tblContact values ('','$fname','$lname','$address','$address2','$city','$state','$zip','$email','$phone','$notes','$stamp')";
		$result=db_query($sql);
			}
}
...
<script>
		...
		window.location.href='http://www.techniart.com/sdgeenergysolutionspartners'
</script>