...
class Ups extends \RocketShipIt\Service\Common
{
    ...
    function upsSoapRequest($request, $methodCode)
    {
        ...
        $license = \RocketShipIt\getUPSDefault('license');
        $username = \RocketShipIt\getUPSDefault('username');
        $password = \RocketShipIt\getUPSDefault('password');

        if (\RocketShipIt\getGenericDefault("debugMode") == 1) {
        ...
        $security = array(
            ...
            "Password" => $password),
            "ServiceAccessToken" => array("AccessLicenseNumber" => $license)
        );
