...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_ambit where sess='$sess' && status='open'";
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_ambit where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_ambit values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_ambit set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
...
$sql="select * from tblotsdetail_ambit where otsID='$o'";
$result=db_query($sql);
