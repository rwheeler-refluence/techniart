...
$subject="PGE Marketplace - techniart.com";
...
#grab unnotified orders
	...
	if($counta){
		while($rowa=mysql_fetch_array($resulta)){
			...
			$fname=$rowa['bill_firstname'];
			$lname=$rowa['bill_lastname'];
			$address1=$rowa['bill_address'];
			$address2=$rowa['bill_address2'];
			$city=$rowa['bill_city'];
			$xship=$rowa['xship'];
			$email=$rowa['email'];
			$campus=$rowa['campus'];
			$type=$rowa['type'];
			$state=$rowa['bill_state'];
			$zip=$rowa['bill_zip'];
...
//			$body.="Sent on: ".date("m/d/Y H:i:s")."\n\n";
			$body.="Your order was received on ".$stmp.". Please find the details of your order below.\n\n";
			$body.="BILLING INFO:\n";
...
TechniArt\n";
						...
						mail($to=$email,$subject="Thank you for your PGE Marketplace order from techniart.com",$body,"From:".$from."");
					}
print("<pre>");
print($body);
print("</pre>");