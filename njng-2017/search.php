...
<?
if($action=='search'){
	print("<a class=\"body_content_style1\" href=\"search.php\"><< Search Again</a><br><br>\n");
	$cnt=0;
...
$resultp=db_query($sqlp);
	...
	<option value="">
	<option value="search.php?cat=<? echo($cat);?>&subcat=<? echo($subcat);?>&sort=replacement_wattage&action=<? echo($action);?>&keyword=<? echo($keyword);?>">Replacement wattage
	<option value="search.php?cat=<? echo($cat);?>&subcat=<? echo($subcat);?>&sort=watts_used&action=<? echo($action);?>&keyword=<? echo($keyword);?>">Wattage used
	<option value="search.php?cat=<? echo($cat);?>&subcat=<? echo($subcat);?>&sort=MSRP&action=<? echo($action);?>&keyword=<? echo($keyword);?>">Price
	<option value="search.php?cat=<? echo($cat);?>&subcat=<? echo($subcat);?>&sort=color_temp&action=<? echo($action);?>&keyword=<? echo($keyword);?>">Color temperature
	<option value="search.php?cat=<? echo($cat);?>&subcat=<? echo($subcat);?>&sort=manuf&action=<? echo($action);?>&keyword=<? echo($keyword);?>">Manufacturer
	</select></div>
	<br>
	<?}?>
	...
	<td width="71"><a href="store-detail.php?ID=<? echo($productID);?>"><img src="<? echo($folder);?><? echo($imageLoc);?>" alt="<? echo($productName);?>" width="<? echo($width);?>" height="<? echo($height);?>" border="0" class="img_stroke"></a></td>
	...
	<? if(!$_SESSION['noshop']){?>
	<a href="store-detail-lightingdiscount.php?ID=<? echo($productID);?>"><img src="pix/b_buy-now_sm.gif" alt="Buy now" width="55" height="19" border="0"></a><br>
	<?}?>
	<a href="store-detail-lightingdiscount.php?ID=<? echo($productID);?>"><img src="pix/b_more-info_sm.gif" alt="Learn more" width="55" height="19" border="0"></a></td>
	</tr></table>
