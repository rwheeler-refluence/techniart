...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_brilliant where sess='$sess' && status='open'";
		$resultc=db_query($sqlc);
		$countc=mysql_num_rows($resultc);
		if(!$countc){
			$sql="insert into tblorderstosend_brilliant values ('','$sess','','','$stamp','open')";
			$result=db_query($sql);
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_brilliant where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_brilliant values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_brilliant set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			$sql="update tblotsdetail_brilliant set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
#print("".$sql."<br />");			
			$result=db_query($sql);
		}
	break;
...
$sql="select * from tblotsdetail_brilliant where otsID='$o'";
$result=db_query($sql);
