...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend3 where sess='$sess' && status='open'";
		$resultc=db_query($sqlc);
		$countc=mysql_num_rows($resultc);
		if(!$countc){
			$sql="insert into tblorderstosend3 values ('','$sess','','','$stamp','open')";
			$result=db_query($sql);
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail3 where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail3 values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail3 set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			$sql="update tblotsdetail3 set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
#print("".$sql."<br />");			
			$result=db_query($sql);
		}
	break;
