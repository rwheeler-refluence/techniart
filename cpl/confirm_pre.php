...
$_SESSION['s_bill_zip']=$bill_zip;
$_SESSION['s_phone']=$phone;
$_SESSION['s_ship_fname']=$ship_fname;
$_SESSION['s_ship_lname']=$ship_lname;
$_SESSION['s_ship_address1']=$ship_address1;
$_SESSION['s_ship_address2']=$ship_address2;
$_SESSION['s_ship_city']=$ship_city;
$_SESSION['s_ship_state']=$ship_state;
?>
<?
$ship_zip=$_POST['ship_zip'];
$sql="select * from tblTerritory_aep where zip='$ship_zip'";
#print("".$sql."<br>");
$result=db_query($sql);
...
if($countzip<1){echo "<SCRIPT LANGUAGE='JavaScript'>
		...
		window.location.href='https://www.techniart.com/cpl/orderform.php'
        </SCRIPT>";}
?>
		...
		if(!$_SESSION['zip']){
			...
			if($countz>0){
				while($rowz=mysql_fetch_array($resultz)){
					...
					$sqlu="update tblotsdetail_cpl set price='$disct_price' where otsdetailID='$otsdetailID'";
					#print($sqlu);
...
<input type=hidden name="bill_zip" value="<? echo($bill_zip); ?>" />
<input type=hidden name="email" value="<? echo($email); ?>" />
<input type=hidden name="phone" value="<? echo($phone); ?>" />
<input type=hidden name="bill_county" value="<? echo($bill_county); ?>" />
