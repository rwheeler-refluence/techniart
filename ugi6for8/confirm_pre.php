...
$instr=$_POST['instr'];
$order=$ship_fname . ',' . $ship_lname . ',' . $ship_address1 . ',' . $ship_address2 . ',' . $ship_city . ',' . $ship_state . ',' . $ship_zip . ',' . $fname . ',' . $lname . ',' . $address1 . ',' . $address2 . ',' . $city . ',' . $state . ',' . $zip . ',' . $bulbs . ',' . $incan . ',' . $email . ',' . $why . ',' . $email_opt . ',' . $otsID;
?>
<?
$ship_zip=$_POST['ship_zip'];
...
if($countzip<1){echo "<SCRIPT LANGUAGE='JavaScript'>
	window.alert('The shipping zip code you entered does not qualify!')
		window.location.href='https://www.techniart.us/ugi6for8/orderform.php'
        </SCRIPT>
        ";
		die();}
