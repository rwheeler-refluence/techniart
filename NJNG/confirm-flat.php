...
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail_PPL where otsID='$o'";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select free_ship,ct_tax_exempt,weight from tblProducts_PPL where productID='$itemNo'";
		...
		if(!$_SESSION['zip']){
			$zz=$_POST['ship_zip'];
			$sqlz="select * from tblDiscounts_PPL LEFT OUTER JOIN tblProducts_PPL on tblDiscounts_PPL.item_no=tblProducts_PPL.modelNumber where tblProducts_PPL.productID='$itemNo' && zip='$zz'";
			#print($sqlz);
			...
			if($countz>0){
				while($rowz=mysql_fetch_array($resultz)){
					...
					$sqlu="update tblotsdetail_PPL set price='$disct_price' where otsdetailID='$otsdetailID'";
					#print($sqlu);
...
#	print("weight: ".$weight."<br>");
	...
	print ("<br><span class=\"body_content_events-private\">This order qualifies for <br>$7 flat rate shipping.</span>");
	}else{ 
	print("".$rate."\n");
...
?>
<form action="process-stop.php" method="POST" >
...
<? echo($ship_city); ?>, <? echo($ship_state); ?><br><? echo($ship_zip); ?><br>United States<br><br></span></td>
</tr></table>
</span></td>
<td width="10">&nbsp;</span></td>
<td width="235" align="left">
...
<td colspan="2" align="center"><img src="pix/pix_trans.gif" width="1" height="10"><br><div align="center"><input type="button" value ="Place Order" onClick="this.form.submit();" /></div></span></td>
</tr></table>
</span></td></tr></table>
</form>
<?}?>
