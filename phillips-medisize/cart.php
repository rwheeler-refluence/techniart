...
switch($action){
	case "add":
		...
		$sqlc1="select * from tblorderstosend_phillips where sess='$sess' && status='open'";
#print("c1: ".$sqlc1."<br>");
		$resultc1=db_query($sqlc1);
		$countc1=mysql_num_rows($resultc1);
		if($countc1<1){
			$sql="insert into tblorderstosend_phillips values ('','$sess','','','$stamp','open')";
#print("c2: ".$sql."<br>");
			$result=db_query($sql);
...
#print("ots: ".$next."<br>");
		...
		#add item to cart
		$sqli1a="select * from tblotsdetail_phillips where itemNo='$itemNo' && otsID='$next'";
#print("".$sqli1a."<br />");
		$resulti1a=db_query($sqli1a);
		$counti1a_g=mysql_num_rows($resulti1a);
#print("counti1: ".$counti1a_g."<br>");
		if($counti1a_g<1){
			$sql21="insert into tblotsdetail_phillips values ('','$next','$itemNo','$quantity1','$price','$desc','')";
...
#print("".$sql21."<br>");
			...
			while($rowi1a=mysql_fetch_array($resulti1a)){
				...
				$qty=$rowi1a['qty'];
				$newqty=$_POST['qty'];
				$sql21="update tblotsdetail_phillips set qty='$newqty' where otsdetailID='$otsdetailID'";
				$result21=db_query($sql21);	
			}
		}
	break;
	...
	case "update":
		...
		for($i=0;$i<count($otsdetailID);$i++){
			if($qty[$i]==0){
				$sql="delete from tblotsdetail_phillips where otsdetailID='$otsdetailID[$i]'";
			}else{
				$sql="update tblotsdetail_phillips set qty='$qty[$i]' where otsdetailID='$otsdetailID[$i]'";
			}
			$result=db_query($sql);
		}
	break;
...
<?
$o=$_SESSION['otsID'];
$sql="select * from tblotsdetail_phillips LEFT OUTER JOIN tblProducts_phillips on tblotsdetail_phillips.itemNo=tblProducts_phillips.productID where tblotsdetail_phillips.otsID='$o'";
#print($sql);
$result=db_query($sql);
...
if(!$count){
 ...
 while($row=mysql_fetch_array($result)){
	...
	/*	if(strlen($_SESSION['zip_qualify'])>0){
			$pr=$row['disct_price'];
			if($pr!==$price){
			$sqlu="update tblotsdetail_phillips set price='$pr' where otsdetailID='$otsdetailID'";
...
#		print("".$calcweight."<br>");
		...
		$sqlfreeship="select free_ship from tblProducts_phillips where productID='$productID'";
...
#5/21
		#shipping actuals
		$sqlship="select * from tblProducts_phillips where productID='$itemNo'";
	//	print("".$sqlship."<br>");
...
#	print("<tr>\n");
	...
	print("</tr>\n");
	print("<tr height=\"3\"><td colspan=\"3\" align=\"right\"></td></tr>");
	$product1 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='1' ");
if (mysql_num_rows($product1)) {
    $P1 = mysql_fetch_assoc($product1);
    $PQ1=$P1['qty']*1;}
$product2 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='2' ");
if (mysql_num_rows($product2)) {
    $P2 = mysql_fetch_assoc($product2);
    $PQ2=$P2['qty']*4;}
$product3 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='3' ");
if (mysql_num_rows($product3)) {
    $P3 = mysql_fetch_assoc($product3);
    $PQ3=$P3['qty']*1;}
	$product4 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='4' ");
	if (mysql_num_rows($product4)) {
    $P4 = mysql_fetch_assoc($product4);
    $PQ4=$P4['qty']*6;}
	$product5 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='5' ");
if (mysql_num_rows($product5)) {
    $P7 = mysql_fetch_assoc($product5);
    $PQ5=$P5['qty']*6;}
	$product6 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='6' ");
	if (mysql_num_rows($product6)) {
    $P6 = mysql_fetch_assoc($product6);
    $PQ6=$P6['qty']*1;}
	$product7 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='7' ");
	if (mysql_num_rows($product7)) {
    $P7 = mysql_fetch_assoc($product7);
    $PQ7=$P7['qty']*6;}
	$product8 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='8' ");
	if (mysql_num_rows($product8)) {
    $P8 = mysql_fetch_assoc($product8);
    $PQ8=$P8['qty']*1;}
	$product9 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='9' ");
	if (mysql_num_rows($product9)) {
    $P9 = mysql_fetch_assoc($product9);
    $PQ9=$P9['qty']*1;}
	$product10 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='10' ");
	if (mysql_num_rows($product10)) {
    $P10 = mysql_fetch_assoc($product10);
    $PQ10=$P10['qty']*1;}

		$product11 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='11' ");
	if (mysql_num_rows($product11)) {
    $P11 = mysql_fetch_assoc($product11);
    $PQ11=$P11['qty']*4;}
		$product12 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='12' ");
	if (mysql_num_rows($product12)) {
    $P12 = mysql_fetch_assoc($product12);
    $PQ12=$P12['qty']*6;}
	$bulblimit=$PQ1+$PQ2+$PQ3+$PQ4+$PQ5+$PQ6+$PQ7+$PQ8+$PQ9+$PQ10+$PQ11+$PQ12;
	$product13 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='26' ");
	if (mysql_num_rows($product13)) {
    $P13 = mysql_fetch_assoc($product13);
    $PQ13=$P13['qty']*1;}
	$product14 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='14' ");
	if (mysql_num_rows($product14)) {
    $P14 = mysql_fetch_assoc($product14);
    $PQ14=$P14['qty']*1;}
	$product15 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='15' ");
	...
	$APSlimit=$PQ13+$PQ14+$PQ15;
		$product242 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='242' ");
if (mysql_num_rows($product242)) {
    $P242 = mysql_fetch_assoc($product242);
    $PQ242=$P242['qty']*1;}
	$product243 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='243' ");
if (mysql_num_rows($product243)) {
    $P243 = mysql_fetch_assoc($product243);
    $PQ243=$P243['qty']*1;}
	$lamplimit=$PQ242+$PQ243;
 ...
 or less</b>.</td>\n");	
	...
	if($lamplimit > '2'){
	print("<td colspan=\"4\" align=\"right\" class=\"style5\">In order to checkout,<br>please adjust your cart<br>to <b>2 total LED 10-Packs or less</b>.</td>\n");	
	print("</tr>\n");
	print("<tr>\n");
	print("</table>\n");
	}else{
...
");}
	if($APSlimit<1){
		$APSlimit='0';}
		if($APSlimit>0){
print("<span class=\"style5\"><br>You currently have <strong>".$APSlimit."</strong> of the allowed <strong>2</strong> APS in your cart.<br>
\n");}
	if($APSlimit > '5'){
		print("<span class=\"body_content_ip\">Please adjust your cart so that you have <strong>2</strong> total APS or less.</span>");}
		if($lamplimit<1){
		$lamplimit='0';}
if($lamplimit>0){		
print("<span class=\"style5\"><br>You currently have <strong>".$lamplimit."</strong> of the allowed <strong>2</strong> LED 10-Packs in your cart.<br>
\n");}
	if($lamplimit > '2'){
		print("<span class=\"body_content_ip\">Please adjust your cart so that you have <strong>2</strong> total LED 10-Packs or less.</span>");}
	?>
<br>
</div></td>
<td></td>
