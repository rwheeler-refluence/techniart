...
switch($action){
	case "add":
		...
		$sqlc="select * from tblorderstosend_phillips where sess='$sess' && status='open'";
		...
		$_SESSION['otsID']=$next;
		#add item to cart
		$sqli="select * from tblotsdetail_phillips where itemNo='$itemNo' && otsID='$next'";
		#print("".$sqli."<br />");
		$resulti=db_query($sqli);
		$counti=mysql_num_rows($resulti);
		if(!$counti){
			$sql2="insert into tblotsdetail_phillips values ('','$next','$itemNo','$quantity','$price','$desc')";
		}else{
			...
			$sql2="update tblotsdetail_phillips set qty='$newqty' where otsdetailID='$otsdetailID'";
		}
#		print($sql2);
...
$sql="select * from tblotsdetail_phillips where otsID='$o'";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		$free_ship="";
		$lbl="";
		$sqlfreeship="select modelNumber from tblProducts_phillips where productID='$itemNo'";
...
if (mysql_num_rows($product5)) {
	...
	$product13 = mysql_query("SELECT qty FROM tblotsdetail_phillips WHERE otsID = '$o' && itemNo='13' ");
...
<tr valign="top">
            <td width="237" align="left"><span class="product_title_sm">Work Shift: *</span></td>
