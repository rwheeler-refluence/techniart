...
<html>
<head>
<title>TechniArt - Phillips Medisize Online Sale</title>
...
<td width="904"><div id="main_content_ip" align="left"><!-- ------------------------------begin products -detail------------------------------ -->
<?
$sqlp="select * from tblProducts_phillips where productID='$ID'";
...
while($rowp=mysql_fetch_array($resultp)){
	...
	$productPub=$rowp['productPub'];
			$loc=$rootDir."phillips-medisize/pix/products/".$imageLoc;
...
<td width="170"><a rel="shadowbox;" href="pix/products/<? echo($imageLoc);?>" class="option" title="<? echo($productName);?>"><img src="<? echo($loc);?>" alt="<? echo($productName);?>" width="200" height="<? echo($height);?>" border="0" class="img_stroke"></a><br>
<? 
if($energy_star=='Yes'){
...
?>
  ...
  <?
  if($extra_image=='Yes'){
  print("<a href=\"pix/products/".$addImage."\" rel=\"shadowbox\"><img src=\"pix/products/thumbnails/".$addImage."\" class=\"img_stroke\"></a>");
	}
?>
</td>
...
<?}?>
<? if($min_start_temp){?>
<b>Dimming:</b> <? echo($min_start_temp);?><br>
<?}?>
<? if($electrical_spec){?>
...
<input type="button" class="btn" value="Buy Now" onClick="this.form.submit();">
<?}?>
<br></span></td>
...
if($replacement_bulb){
	...
	for($r=0;$r<count($split);$r++){
			...
			$sqlnum="select productID,productName from tblProducts_phillips where modelNumber='$mf_nbr'";
