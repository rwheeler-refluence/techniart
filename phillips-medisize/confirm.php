...
$sql="select * from tblotsdetail_phillips where otsID='$o' ORDER BY ItemNo + 0 ASC";
$result=db_query($sql);
...
if(!$count){
	...
	while($row=mysql_fetch_array($result)){
		...
		if(!$_SESSION['zip']){
			$zz=$_POST['ship_zip'];
			$sqlz="select * from tblDiscounts LEFT OUTER JOIN tblProducts_phillips on tblDiscounts.item_no=tblProducts_phillips.modelNumber where tblProducts_phillips.productID='$itemNo' && zip='$zz'";
			#print($sqlz);
			...
			if($countz>0){
				while($rowz=mysql_fetch_array($resultz)){
					...
					$sqlu="update tblotsdetail_phillips set price='$disct_price' where otsdetailID='$otsdetailID'";
					#print($sqlu);
...
$rate = '0.00';
#$sqltest = "SELECT ItemNo FROM tblotsdetail_phillips WHERE otsID='$o' ORDER BY ItemNo + 0 ASC";
#$sqltest = "SELECT ItemNo FROM tblotsdetail_phillips WHERE otsID='$o' ORDER BY ItemNo ASC";
...
#	print("<td colspan=\"5\" align=\"right\">");
		if($state=='WI' || $state=='RI' || $state=='WA' || $state=='MA' || $state=='CA'){
